<template>
	<div class="revisePhone">
		<HeaderTop :title="`修改微信号`"></HeaderTop>
		<div class="revise-account">
			<div class="revise-account-input"><input type="text" :value="value" ref="weixin" placeholder="请输入微信"></div>
		</div>
...
export default {
	...
	methods: {
		...
		userInfo(success,data){
			if(success){
				if(data.code=='0000'){
					this.value=data.data.weixin;
				}
			}
		},
		toPush(){
			let params = {
				weixin: this.$refs.weixin.value
			}
			if(this.isEmpty(params.weixin)){
				Toast('微信号码不能为空');
				return false;
			}
