	...
	<div class="password-retrieval">
        <div class="replace-header">
			...
			<div class="header-title">
                {{titleFonts}}
                <div class="small-title">手机号绑定</div>
            </div>
        ...
        <div class="main" v-if='step2'>
            <div class="content-list flex-b">
                <div class="content-list-left">新手机号码</div>
				<div class="content-list-right">
					<input type="number" maxlength="11" placeholder="请输入新手机号码" ref="newPhone" oninput="if(value.length>11)value=value.slice(0,11)" @input='checkStep2()'>
				</div>
			</div>
            <div class="content-list flex-b">
                <div class="content-list-left">新手机验证码</div>
				<div class="content-list-right flex-b">
					<input class='code-input' type="number" maxlength="4" placeholder="请输入验证码" v-model="newPhoneCode" oninput="if(value.length>4)value=value.slice(0,4)" @input='checkStep2()'>
                    <button class='code-btn' @click="sendMessage2($event)">发送验证码</button>
				</div>
			</div>
            <div class="submitBlue">
...
export default {
	...
	methods: {
        ...
        checkStep2(){
            if(this.$refs.newPhone.value !== '' && this.newPhoneCode !== ''){
        ...
        sendMessage2(e) {
			...
			let time = 60;
			if (time == 60) {
				this.ServerAPI.postAPI(this.API.sendSmsNewPhone, params, (success, data) => {
                    Toast.clear()
					if (success) {
						if (data.code == '0000') {
							Toast("验证码发送成功");
        ...
        toPush(){
            ...
            if (this.isEmpty(params.newPhone)) {
				Toast('请输入新手机号码');
				return false;
			}
            if (this.isEmpty(params.newCode)) {
				Toast('请输入验证码');
				return false;
			}
            const toast = Toast.loading({
            ...
            this.ServerAPI.postAPI(this.API.updatePhone3, params, (success, data) => {
                Toast.clear()
				if (success) {
					if (data.code == '0000') {
						Toast('提交成功');
