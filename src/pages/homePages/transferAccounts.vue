<template>
	<div class="transferAccounts">
		<HeaderTop :title="`转账`"></HeaderTop>
		<div class="or-log top-line">
			<div class="main-balance">
				<!-- <img class="tlog-img" src="../../../static/images/transfer-balance-icon.png" alt=""> -->
                <lottie class='lottie-icon' style='width:46px;height:unset;margin: 5px 0 0 -10px;padding:0;' :options="defaultOptions" :animCreated="handleAnimation"/>
				<span>中心钱包：</span>
		...
		<div class="trans-pad">
			...
			<div class="list-bg">
				<input placeholder="请输入转账金额" type="number" v-model="moneyValue" @input="checkInput()">
...
export default {
	...
	methods:{
		...
		loadTop(){
			...
			this.ServerAPI.postAPI(this.API.gameList,params,this.gameList);
			this.balance(1,-1);
		},
		...
		// 	this.dataList[0].values.map((v,k)=>{
		// 		if(v==this.account){
		// 			this.index=k;
		// 		}
		// 	});
		// 	this.paramId = this.accountIdListShow[this.index];
		// },
		toPush(){
			this.showLoading= true
		...
		inputMax(){
            ...
            console.log(typeof this.moneyValue)
            this.checkInput()
		},
		gameList(success,data){
			if(data.code=='0000'){
				...
				for(let k = this.gameShow.length -1; k >= 0 ; k--){
		...
		withdrawAll(){
			...
			this.ServerAPI.postAPI(this.API.gameWithdrawAll,params,this.gameWithdrawAll);
		},
